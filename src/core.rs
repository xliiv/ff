...
pub fn add(file_path: &str, home_dir: &str, sync_dir: &str) -> Result<(), String> {
	...
	if let Err(e) = unix_fs::symlink(&abs_dst, &file_path) {
		println!("Can't symlink moved file {} to {} ({})", abs_dst, file_path, e);
...
pub fn remove(symlinked: &str) -> Result<(), String> {
	...
	if let Err(e) = fs::rename(&regular_file, &symlinked) {
		return Err(
			format!("Can't move file {:?} to {:?} ({})", &regular_file, &symlinked, e)
		);
	}
	Ok(())
}
...
///
/// `symlink_file(WalkDir::DirEntry("/home/joe/sync-dir/.bashrc"), "/home/joe/sync-dir", "/home/joe")`
///
...
pub fn symlink_file(sync_file: DirEntry, sync_dir: &str, home_dir: &str) -> Result<(), String> {
	...
	let src_path = sync_file.path().to_str()
	...
	let user_file_dir = user_file.parent()
		.and_then(|p| p.to_str())
