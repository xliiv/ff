...
fn action_add(file_paths: &Vec<&str>, space_dir: &str, config: Config) -> Result<(), String> {
	let home_dir = env::home_dir()
		.ok_or("Can't get home dir")?;
	let home_dir = home_dir.to_str()
		.ok_or("Can't convert home dir to str")?;
	let sync_dir = with_space_dir(space_dir, config)?;
	let sync_dir = sync_dir.to_str()
		.ok_or("Can't convert home dir to str")?;
	...
	match matches.subcommand_name() {
		...
		Some("add") => {
			if let Some(ref matches) = matches.subcommand_matches("add") {
				let file_paths: Vec<_> = matches.values_of("file-path").expect("Can't get file paths").collect();
		...
		Some("remove") => {
			if let Some(ref matches) = matches.subcommand_matches("remove") {
				let file_paths: Vec<_> = matches.values_of("file-path").expect("Can't read file paths to remove").collect();
